...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         // 308 - SPELL_AURA_MOD_CRIT_CHANCE_FOR_CASTER implement in Unit::isSpellCrit
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (triggeredSpellInfo)
    {
        if (Unit* triggerCaster = triggeredSpellInfo->NeedsToBeTriggeredByCaster() ? caster : target)
        {
            ...
            sLog->outString("AuraEffect::HandlePeriodicTriggerSpellAuraTick: Spell %u is triggering %u on target %u by caster %u", GetId(), triggeredSpellInfo->Id, target->GetGUIDLow(), triggerCaster->GetGUIDLow());
        }
    }
    else
    {
